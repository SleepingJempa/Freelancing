...
void MainMenu(){
  ...
  cout << "2.Sign in as Shopkeeper" << endl;
  cout << "3.Credit" << endl;
  RedColor;
  cout << "4.Exit" <<endl;
  BlackColor;
...
void Checkout(){
  ...
  //   while (opencategorylist >> category){
  //     cout << category << endl;
  //     category += ".txt";
  //     while (checkout >> myProduct.id >> myProduct.name >> myProduct.amount >> myProduct.price){
  //       ofstream tempCategory("temp.txt");
  //       ifstream opencategory(category);
  //       while (opencategory >> id >> name >> amount >> price){
  //         if (id == myProduct.id && name == myProduct.name){
  //           tempCategory << myProduct.id << ' ' << myProduct.name << ' ' << abs(myProduct.amount - amount) << ' ' << myProduct.price << endl;
  //         }
  //         else {
  //           tempCategory << id << ' ' << name << ' ' << amount << ' ' << price << endl;
  //         }
  //       }
  //       opencategory.close();
  //       tempCategory.close();
  ...
  // remove ("Cart.txt");
  // ifstream cart("Cart.txt");
  // while (cart >> myProduct.id >> myProduct.name >> myProduct.amount >> myProduct.price){
  //
  // }
  // ifstream list("CategoryList.txt");
  // while (list >> category){
  //   category += ".txt";
  //   cout << category << endl;
  ...
  //   while (openCategory >> id >> name >> amount >> price){
  //     if (id == myProduct.id && name == myProduct.name){
  //       temp << myProduct.id << ' ' << myProduct.name << ' ' << abs(myProduct.amount - amount) << ' ' << myProduct.price << endl;
  //     }
  //     else {
  //       temp << id << ' ' << name << ' ' << amount << ' ' << price << endl;
  //     }
  //   }
  // temp.close();
  // openCategory.close();
...
void AddCategory(){
  ...
  if (decision == 'y'){
    ...
    // DWORD attributes = GetFileAttributes("CategoryList.txt");
    // SetFileAttributes("CategoryList.txt", attributes + FILE_ATTRIBUTE_HIDDEN);
...
void EditCategories(){
  ...
  copyofCategory += ".txt";
  NewCategoryName += ".txt";
  const char * oldcategory = copyofCategory.c_str(); // function remove va rename az const char estefade mikonan,
  const char * newcategory = NewCategoryName.c_str(); // pas bayad hatman tabdil besan be const char.
