...
def ex1(acn1, acn2, acn3, imd_acn1, imd_acn2, init_amount, transact_log):
    ...
    acn = [init_amount] * 3
    imd = [0] * 3
    debt = [[0] * 3 for i in range(2)]

    def transaction(trans_object):
        fr = acn_index[trans_object[0][0]]
        to = acn_index[trans_object[0][1]]

        amount = trans_object[1]
        the_imd = imd_index[trans_object[2]]
        fee = trans_object[3]

        print('Processing transaction sending {}Ħ from player {} to player {} with fee {}'.format(
            amount,
            trans_object[0][0],
            trans_object[0][1],
            fee
        ))
        print('Agent: {}'.format(trans_object[2]))

        # Ok
        if acn[fr] >= amount * fee // 100 + amount:
            print('Transaction was successful.')
            acn[fr] -= amount * fee // 100 + amount
            imd[the_imd] += amount * fee // 100
            acn[to] += amount
        else:
            print('Transaction failed')
            acn[fr] -= amount * fee // 100

            if acn[fr] < 0:
                print('Player {} owes {}Ħ to the agent {}'.format(
                    trans_object[0][0],
                    amount * fee // 100,
                    trans_object[2]
                ))
                print('Agent {} got {}Ħ'.format(
                    trans_object[2],
                    amount * fee // 100 + acn[fr]
                ))
                # debts
                imd[the_imd] += amount * fee // 100 + acn[fr]
                debt[the_imd][fr] = acn[fr]
                acn[fr] = 0
                pass
            else:
                print('Agent {} got {}Ħ'.format(
